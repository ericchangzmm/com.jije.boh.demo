...
_$.findNodes = function(paren, elem)
{
    for (var i = 0; i < paren.children.length; i++) 
    {
        ...
        {
            _$.findNodes(paren.children[i], elem);
        }
    }
}
...
_$.getTarget = function(event)
{
    var e = this.getEvent(event);
    return e.target || e.srcElement;
}
_$.toClick = function(e, param)
{
    var pren = $(param[0]);
    if (pren.length != 0) 
    {
...
_$.getTime = function(con)
{
    ...
    switch (param.timeMode)
    {
        ...
        case "YYYY":
            ...
            break;
        case "MM":
            unDate = month < 10 ? "0" + month : month;
            break;
        case "DD":
			unDate = dateNow < 10 ? "0" + dateNow : dateNow;
            break;
        case "hh":
			unDate = hours < 10 ? "0" + hours : hours;
            break;
        case "mm":
			unDate = minute < 10 ? "0" + minute : minute;
            break;
        case "ss":
			unDate = second < 10 ? "0" + second : second;
            break;
    }
	if (param.weekday) 
	{
...
_$.cookie = 
{
    /**
     * 添加cookie
     * name,(名称) value(值), days(保存天数), path(路径),domain(权限),code(是否编码)
     * @param {Object} param
     */
    set: function(param)
    {
        ...
        {
            var date = new Date();
            param.days = param.days || 7;
            date.setDate(date.getDate() + param.days);
            var cookieValue = param.value;
            if (param.code != false) 
            {
                cookieValue = encodeURIComponent(param.value)
            }
            var str = param.name + "=" + cookieValue + "; expires=" + date.toGMTString();
            if (param.path) 
            {
                str += ";path=" + param.path;
            }
     ...
     * @param {Object} nodeName cookie子健名
     */
    get: function(name, nodeName)
    {
        if (document.cookie.length) 
        {
            var start = document.cookie.indexOf(name + '='), str = null, end = 0;
            if (start !== -1) 
            {
                ...
                var op = document.cookie.substring(start, end).replace(/[+]/g, '%20');
                str = decodeURIComponent(op);
                if (nodeName && str.indexOf("=") != -1) 
                {
                    str = str.split("&");
                    
                    for (var i = 0; i < str.length; i++) 
                    {
                        if (str[i].indexOf("=") != -1 && str[i].split("=")[0] == nodeName) 
                        {
...
_$.moveCore = 
{
    startMove: function(obj, json, fnEnd, moveTimer)
    {
        if (!moveTimer) 
        {
            moveTimer = 30;
        ...
        {
            _$.moveCore.doMove(obj, json, fnEnd);
        }, moveTimer);
        var oDate = new Date();
        
        if (oDate.getTime() - obj.lastMove > moveTimer) 
        {
            _$.moveCore.doMove(obj, json, fnEnd);
        }
    },
    ...
    doMove: function(obj, json, fnEnd)
    {
        ...
        for (attr in json) 
        {
            if (attr == 'opacity') 
            {
                iCur = parseInt(100 * parseFloat(_$.moveCore.getStyle(obj, 'opacity')));
            }
            else 
            {
                iCur = parseInt(_$.moveCore.getStyle(obj, attr));
            }
